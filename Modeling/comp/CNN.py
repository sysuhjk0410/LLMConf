...
# Columns 1 to 10 are configuration parameters, the remaining columns are 16 evaluation metrics
...
class CNNModel(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.conv1 = nn.Conv1d(1, 16, kernel_size=3, padding=1)  # Convolution layer 1
        self.conv2 = nn.Conv1d(16, 32, kernel_size=3, padding=1)  # Convolution layer 2
        self.fc1 = nn.Linear(32 * input_dim, 64)  # Fully connected layer 1
        self.fc2 = nn.Linear(64, output_dim)  # Output layer

    def forward(self, x):
        x = x.unsqueeze(1)  # Add an extra dimension (batch_size, 1, input_dim)
        x = torch.relu(self.conv1(x))  # Activation function ReLU
        x = torch.relu(self.conv2(x))  # Activation function ReLU
...
# Train and prediction function construction
...
print("All models trained and functions created.")
